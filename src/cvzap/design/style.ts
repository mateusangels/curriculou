...
export function buildCSS(d: DesignConfig): string {
  ...
  return `
  .cv-root { font-family: ${r.fontCss}; font-size: ${r.basePx}px; line-height: 1.5; color: ${r.mainText}; background: ${r.bgColor}; ${dual ? `display:flex; flex-direction:${d.sidebarLado === 'right' ? 'row-reverse' : 'row'}; min-height: 1123px;` : ''} }
  ...
  .cv-main h2 { font-size: ${r.basePx * 1.15}px; font-weight: 700; letter-spacing: .6px; text-transform: uppercase; color: ${r.sectionTitle}; margin: ${dual ? '20px' : '22px'} 0 10px; ${r.headerBorder ? `border-bottom: 2px solid ${r.sectionBorder}; padding-bottom: 5px;` : ''} }
  .cv-main h2:first-child { margin-top: 0; }

  .cv-contact-row { display: flex; gap: 8px; align-items: flex-start; margin-bottom: 7px; font-size: ${r.basePx * 0.95}px; word-break: break-word; color: ${dual ? r.sidebarMuted : r.mainMuted}; }
  .cv-contact-row svg { width: 1em; height: 1em; margin-top: 2px; flex-shrink: 0; opacity: .85; }
  .cv-info { font-size: ${r.basePx * 0.95}px; margin-bottom: 5px; color: ${dual ? r.sidebarMuted : r.mainMuted}; }
  .cv-info b { color: ${dual ? r.sidebarText : r.mainText}; font-weight: 600; }

  .cv-single-contact { display: flex; flex-wrap: wrap; gap: 4px 18px; margin-top: 10px; font-size: ${r.basePx * 0.95}px; color: ${r.mainMuted}; }
  .cv-single-contact .cv-contact-row { margin-bottom: 0; }

  .cv-skill { display: flex; justify-content: space-between; align-items: center; gap: 10px; margin-bottom: 7px; font-size: ${r.basePx * 0.95}px; }
  ...
  .cv-chip { border: 1px solid ${r.accent}55; color: ${r.accent}; border-radius: 4px; padding: 2px 9px; font-size: ${r.basePx * 0.92}px; }

  .cv-item { margin-bottom: 12px; ${r.layout === 'dual' ? '' : ''} }
  .cv-item .cv-top { display: flex; justify-content: space-between; align-items: baseline; gap: 10px; }
  .cv-item .cv-role { font-weight: 700; font-size: ${r.basePx * 1.08}px; color: #222; }
  .cv-item .cv-org { font-size: ${r.basePx * 0.98}px; color: ${r.accent}; font-weight: 600; }
  .cv-item .cv-per { font-size: ${r.basePx * 0.85}px; color: ${r.mainMuted}; white-space: nowrap; font-weight: 500; }
  .cv-item .cv-desc { margin-top: 3px; color: #4a5060; ${r.bulletIndent ? 'padding-left: 14px; position: relative;' : ''} }
