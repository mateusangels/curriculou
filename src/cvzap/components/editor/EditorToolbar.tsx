...
import { Sparkles, Target, Languages, LayoutGrid, Palette, SlidersHorizontal, Download, Undo2, Redo2, MessageSquareText, Moon, Sun, ArrowLeft, Save, FolderOpen, Menu, X, Upload } from 'lucide-react';
...
// botão "ghost": sem borda própria — o grupo (pílula) é quem dá o contorno.
function Btn({ children, onClick, title, disabled }: { children: React.ReactNode; onClick?: () => void; title?: string; disabled?: boolean }) {
  return (
    <button onClick={onClick} title={title} disabled={disabled}
      className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm font-medium text-slate-600 transition hover:bg-white hover:shadow-sm disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:shadow-none dark:text-slate-200 dark:hover:bg-slate-700">
      {children}
    </button>
  );
}

// container que agrupa botões relacionados num bloco (pílula)
function Grupo({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-0.5 rounded-xl bg-slate-100/80 p-1 dark:bg-slate-800/60">
      {children}
    </div>
  );
}
  ...
  return (
    <header className="border-b border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
      <div className="flex items-center gap-2 px-3 py-2.5">
        ...
        {/* DESKTOP: blocos agrupados por função */}
        <div className="hidden flex-1 flex-wrap items-center gap-2 md:flex">
          {/* Conteúdo / IA */}
          <Grupo>
            <Btn onClick={() => p.emBreve('Importar currículo')} title="Importar de um arquivo (em breve)"><Upload className="h-4 w-4" /> Importar</Btn>
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {p.onSalvar && (
              <button onClick={p.onSalvar} title="Salvar na minha conta"
                className="flex items-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-100 dark:border-indigo-500/30 dark:bg-indigo-500/10 dark:text-indigo-300">
                <Save className="h-4 w-4" /> Salvar
              </button>
            )}
            <button onClick={p.onChat} title="Assistente (chat)"
              className="flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-indigo-500 to-indigo-600 px-3 py-2 text-sm font-semibold text-white hover:opacity-95">
              <MessageSquareText className="h-4 w-4" /> Assistente
            </button>
            <button onClick={p.onBaixar} title="Baixar PDF"
              className="flex items-center gap-1.5 rounded-lg bg-amber-400 px-3.5 py-2 text-sm font-bold text-amber-950 shadow-sm hover:bg-amber-300">
              <Download className="h-4 w-4" /> Baixar
            </button>
            <button onClick={p.onToggleDark} className="ml-1 rounded-lg p-2 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800" title="Tema">
      ...
      {menu && (
        <div className="grid grid-cols-2 gap-2 border-t border-slate-200 px-3 py-3 md:hidden dark:border-slate-700">
          ...
          <MenuItem onClick={fechando(() => p.emBreve('Importar currículo'))}><Upload className="h-4 w-4" /> Importar</MenuItem>
